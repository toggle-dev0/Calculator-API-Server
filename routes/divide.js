    ...
    const division = (firstOperand, secondOperand) => {
        ...
        if (validOperands(firstOperand, secondOperand)) {
            let result = {
                ...
                // Making use of an anonymous method (function in an object) that checks if the value of the second operand is zero
                "result": (() => {

                    // If the comparison/condition is true, the division takes place
                    if (secondOperand !== 0) {
                        return `Result: ${firstOperand / secondOperand}`
                    }

                    // Otherwise, the division does not take place 
